	...
	function visitNode(node: typescript.Node): typescript.VisitResult<typescript.Node> {
		if (shouldMutateModuleSpecifier(node)) {
			if (typescript.isImportDeclaration(node)) {
				...
				return typescript.factory.updateImportDeclaration(node, node.decorators, node.importClause, newModuleSpecifier, node.assertClause);
			} else if (typescript.isExportDeclaration(node)) {
				const newModuleSpecifier = typescript.factory.createStringLiteral(`${node.moduleSpecifier.text}.js`);
				return typescript.factory.updateExportDeclaration(node, node.decorators, node.isTypeOnly, node.exportClause, newModuleSpecifier, node.assertClause);
			}
		}
